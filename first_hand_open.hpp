...
bool is_openhand_loveletter(const belief_state& bs) {
  return bs.open_e().has_value() && bs.open_s().has_value() && !bs.hand_s[1].has_value();
}
...
bool is_openhand_loveletter_his(int open[3], std::string his0, std::string his1, bool rnd) {
  ...
  // 両方 > 0 なら双方の手札が互いに既知。
  return bs0.open_e().has_value() && bs1.open_e().has_value() && !bs0.hand_s[1].has_value() && !bs1.hand_s[1].has_value();
}
#endif
