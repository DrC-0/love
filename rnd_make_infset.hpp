...
void rnd_ds_draw(node &n);
void rnd_ds_play(node &n, int c);
void rnd_ds_wizard(node &n);
void rnd_ds_wizard_self(node &n);
...
void rnd_ds_draw(node &n){
    ...
    if(all_elements_test) {
      ...
      cout << "rnd_ds_draw history add" << endl;
      x.history.emplace_back(n.rnd_his.get_hash_value());
    ...
    x.play = true;
#ifdef CHECK_STR
    cout << "rnd_ds_draw check_str" << endl;
...
void rnd_ds_play(node &n, int c){
    ...
    switch(c) {
        case 1:
          ...
          if(n.hand2 > 1 && n.barrier2 == false){ end_points++;
            ...
            for(int i = 1; i < 9; i++){
              if(n.deck[i-1] == 0) { continue; }
              work_do_action ds_w;
              ds_w.randsol_wprob = prob_win;
              n.do_action(4, i, ds_w);
              rnd_ds_draw(n);
              n.undo_action(4, i, ds_w);
            }
            return;
          }
          break;
