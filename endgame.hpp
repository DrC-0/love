...
std::array<bool, 14> get_legal_action(const State& s){
    ...
    actions[10] = wiz; actions[11] = wiz;
    actions[12] = s.in(6); actions[13] = s.in(7);
    return actions;
}
...
prob_endgame game_result_prob(const State& s, const Action a){
    prob_endgame res;
    res.win = 0; res.draw = 1; res.lose = 0;
    if(a == Wiz_self && s.other(5) == 8){
        res.lose = 1; res.draw = 0; return res;
    }
    if(a == Crown || a == Priest || a == General || a == Minister || s.barrier){
        return res;
    }
    if(Sol_2 <= a && a <= Sol_8){
        res.win = s.hand_e_prob(int(a)+1); res.draw -= res.win; return res;
    }
    if(a == Knight){
        int hand = s.other(3);
        int last_g = 0;
    ...
    if(a == Wiz_enemy){
        res.win = s.hand_e_prob(7); res.draw -= res.win; return res;
    }
    return res;
}
...
std::vector<State> get_state_by_action(const State& s, const Action a){
    ...
    if(s.barrier){
        ...
        else if(a == Minister){s2.lt5_flag_e = true;}
    ...
    else if(a == Minister){s2.lt5_flag_e = true; states = set_hand(s2, hand);}
    else if(a == Knight){
        if(s.hand_e(hand-1)){
            s2.open_flag_s = hand;
            s2.open_flag_e = hand;
...
double exp_reward2(const int hash){
    ...
    if(enemyhand > 0){
        ...
        else {decode_hash(hash).print(); std::cout << hash << std::endl; exit(1);}
    }
...
double exp_reward(const int hash){
    ...
    for(int i = 0; i < ACTION_COUNT; ++i) {
        ...
        double w, d, l;
        w = result.win; d = result.draw; l = result.lose;
...
bool abs_win(const State &s){
    ...
    for(int i = 0; i < ACTION_COUNT; ++i) {
        ...
        if(result.win + result.lose == 1.0){
            ...
            dreward /= (s.hand_e_prob(last[0]-1) + s.hand_e_prob(last[1]-1) + s.hand_e_prob(last[2]-1));
            if (result.win + result.draw * dreward >= 1.0) return true;
        }else if(s.barrier && Sol_3 <= Action(i) && Action(i) <= Sol_8){ continue;
        }else{
