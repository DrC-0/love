...
#endif
public:
  void set_prob_action(double v) noexcept {
    prob_action = (float)v;
  }
  double get_prob_action() const noexcept {
    return (double)prob_action;
  }
  void set_pi_i(double v) noexcept {
    pi_i = (float)v;
  }
  double get_pi_i() const noexcept {
    return (double)pi_i;
  }
#ifdef CFR
  void set_sum_i(int i, double v) noexcept {
    sum_i[i] = (float)v;
  }
  double get_sum_i(int i) const noexcept {
    return (double)sum_i[i];
  }
  void set_regret(int i, double v) noexcept {
    regret[i] = (float)v;
  } /**/
  double get_regret(int i) const noexcept {
    return (double)regret[i];
  } /**/
#endif
#ifdef BEST_RESPONSE
  void set_exp_reward(int i, double v) noexcept {
    exp_reward[i] = (float)v;
  }
  double get_exp_reward(int i) const noexcept {
    return (double)exp_reward[i];
  }
#endif
#ifdef CFR_SELF_PLAY
  void add_sum_i(int i) noexcept {
    sum_i[i]++;
  }
  void set_sum_i(int i, unsigned int v) noexcept {
    sum_i[i] = v;
  }
  unsigned int get_sum_i(int i) const noexcept {
    return sum_i[i];
  }
  void set_value(int i, double v) noexcept {
    value[i] = (float)v;
  } /**/
  double get_value(int i) const noexcept {
