    ...
    if(n.turn == br_player){
      ...
      if(table_exp_reward_it == table_exp_reward.end()){
        ...
        if(table_exp_reward_it == table_exp_reward.end()){
          ...
          output_hash_history(n.org_his.get_hash_value(), false); cout << endl;
          terminate();
        }
      }
  ...
  if(m.empty()){
    ...
    for(vector<string>::iterator it = m.begin(); it != m.end(); it++){
      ...
      if(head > his_p.size()){
        ...
        cout << " n.his_p : "; output_hash_history(n.org_his_p[br_player].get_hash_value(), false);
  ...
  for(std::vector<std::string>::iterator h = mem_all_history.begin(); h != mem_all_history.end(); ++h){
    ...
    switch(next_sign){
      case 0: {
        ...
        break; }
      case 1: {
        if(n_exp.barrier2 == false){ soldior_infsets++;
          bool candidate[8] = {false, false, false, false, false, false, false, false};
