#ifndef INFSET_DFS_RND_HPP
#define INFSET_DFS_RND_HPP
  ...
  if(m.empty()) {
    ...
    for(vector<string>::iterator it = m.begin(); it != m.end(); it++) {
      ...
      if(head > his_p.size()) {
        ...
        cout << " n.his_p : ";
        output_hash_history(n.rnd_his_p[br_player].get_hash_value(), true);
      ...
      if(sign == 0) {
        ...
        if((c2a01 == 1 && to == 1 && n.turn == 0) || (c2a01 == 2 && to == 0 && n.turn == 1)) {
          ...
          if(n.barrier2 == false) {
            ...
            head++;
            string next_action = rph.get_action((unsigned char)his_p[head]);
