...
cfrorg: cfr_org.cpp make_infset.hpp bf_position.hpp $(COMMON_SRCS) $(COMMON_HDRS)
	g++ -std=c++20 -Wall -O2 $(COMMON_DEFS) -DCFR $(COMMON_SRCS) bf_position.hpp cfr_org.cpp -o $@

cfrorgd: cfr_org.cpp make_infset.hpp bf_position.hpp $(COMMON_SRCS) $(COMMON_HDRS)
	g++ -std=c++20 -Wall -O2 $(COMMON_DEBUG_DEFS) -DCFR $(COMMON_SRCS) bf_position.hpp cfr_org.cpp -o cfrorg

watch: cfr_org.cpp bf_position.hpp $(COMMON_SRCS) $(COMMON_HDRS)
	g++ -std=c++20 -Wall -O2 $(COMMON_DEFS) -DCFR $(COMMON_SRCS) bf_position.hpp watch_cfr.cpp -o $@
...
win: infset_iswin.cpp save_load_abshistory.hpp bf_position.hpp $(COMMON_SRCS) $(COMMON_HDRS)
	g++ -std=c++20 -Wall -O2 $(COMMON_DEFS)  $(COMMON_SRCS) bf_position.hpp infset_iswin.cpp -o $@


comp: compare_abscfr.cpp save_load_abshistory.hpp bf_position.hpp $(COMMON_SRCS) $(COMMON_HDRS)
	g++ -std=c++20 -Wall -O2 $(COMMON_DEFS)  $(COMMON_SRCS) bf_position.hpp compare_abscfr.cpp -o $@
