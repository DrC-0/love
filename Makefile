...
# ローカル同士のシャドーイングだけを警告する
COMMON_WARN = -Wall -Wextra -Wshadow=local
...
cfr: cfr.cpp $(COMMON_SRCS) $(COMMON_HDRS)
	g++ -std=c++20 $(COMMON_WARN) -O2 $(COMMON_DEFS) -DCFR $(COMMON_SRCS) cfr.cpp -o $@
...
CPPCHECK_FLAGS = --enable=warning,style,performance,portability --std=c++20 --language=c++ \
                 --inline-suppr --suppressions-list=.cppcheck-suppressions -q
cppcheck:
	cppcheck $(CPPCHECK_FLAGS) $(COMMON_DEFS) *.cpp
