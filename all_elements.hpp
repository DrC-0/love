...
void all_put_hide_card(string &h, unsigned long int head, node &n) {
  string action = oph.get_action((unsigned char)h[head]);
...
void all_draw(string &h, unsigned long int head, node &n) {
  ...
  if(n.org_his_p[n.turn].get_hash_value() == h) {
    all_history.emplace_back(n.org_his.get_hash_value());
    return;
  }
...
void all_play(string &h, unsigned long int head, node &n, int c) {
  ...
  case 5: {
    ...
    int trash = (c2w / 10) % 10;
    int draw = c2w % 10;
    action = oph.get_action((unsigned char)h[0]);
    ...
    if((c2a01 == 1 && to == 1 && n.turn == 0) || (c2a01 == 2 && to == 0 && n.turn == 1)) {
      ...
      if(n.barrier2 == false) {
        ...
        head++;
        string next_action = oph.get_action((unsigned char)h[head]);
  ...
  case 6:
    if(n.barrier2 == false) {
      ...
      int give = c2t / 10;
      int take = c2t % 10;
      string p = oph.get_action((unsigned char)h[0]);
  ...
  head++;
  if(head >= h.size()) return;
  string next = oph.get_action((unsigned char)h[head]);
