...
// if constexpr で捨てられる側でも非依存名の名前解決は起きるので、
// rnd しか使わない翻訳単位 (br_rnd.cpp) でも宣言だけは見えている必要がある。
// 捨てられた分岐はコード生成されないので、定義が無くてもリンクは通る。
extern double soldior_prob[8];

struct org_elements {
  static Org_Perfect_Hash &ph() {
    return oph;
  }
  static constexpr int max_hash_value = ORG_MAX_HASH_VALUE;
  static std::string his(const node &n) {
    return n.org_his.get_hash_value();
  }
  static std::string his_p(const node &n, int turn) {
    return n.org_his_p[turn].get_hash_value();
  }
  static constexpr bool soldior_is_decision = true;
};

struct rnd_elements {
  static Rnd_Perfect_Hash &ph() {
    return rph;
  }
  static constexpr int max_hash_value = RND_MAX_HASH_VALUE;
  static std::string his(const node &n) {
    return n.rnd_his.get_hash_value();
  }
  static std::string his_p(const node &n, int turn) {
    return n.rnd_his_p[turn].get_hash_value();
  }
...
struct all_elements_walker {
  static void put_hide_card(string &h, unsigned long int head, node &n) {
    string action = P::ph().get_action((unsigned char)h[head]);
  ...
  static void play(string &h, unsigned long int head, node &n, int c) {
      ...
      if constexpr(P::soldior_is_decision) {
        if(n.barrier2 == false) {
          if(n.turn == br_player) {
            if(P::his_p(n, n.turn) == h) {
              all_history.emplace_back(P::his(n));
              soldior_prob[n.hand2 - 1] += n.npi[1 - n.turn][n.depth] * n.npi[2][n.depth];
              return;
            } else if(head >= h.size()) {
              return;
            }
            ...
            if(action.size() > 1) {
              ...
              int choice = c2t % 10;
              if(choice + 1 != 1 && choice + 1 == n.hand2) return;
    ...
    case 5: {
      ...
      int trash = (c2w / 10) % 10;
      int draw = c2w % 10;
      action = P::ph().get_action((unsigned char)h[0]);
      ...
      if((c2a01 == 1 && to == 1 && n.turn == 0) || (c2a01 == 2 && to == 0 && n.turn == 1)) {
        ...
        if(n.barrier2 == false) {
          ...
          if(n.hand2 == 8) return;
          work_do_action all_w3 = all_w0;
          head++;
          n.do_action(6, draw + 1, all_w3);
          wizard_self(h, head, n);
          n.undo_action(6, draw + 1, all_w3);
          head--;
        } else {
          head++;
          string next_action = P::ph().get_action((unsigned char)h[head]);
          ...
          if(ctoa2_a == 3) {
            ...
            // この分岐には int draw があり、同名のメンバ関数を隠すので修飾する
            all_elements_walker::draw(h, head, n);
    ...
    case 6:
      if(n.barrier2 == false) {
        ...
        int give = c2t / 10;
        int take = c2t % 10;
        string p = P::ph().get_action((unsigned char)h[0]);
    ...
    head++;
    if(head >= h.size()) return;
    string next = P::ph().get_action((unsigned char)h[head]);
...
inline void all_put_hide_card(string &h, unsigned long int head, node &n) {
  all_walker::put_hide_card(h, head, n);
}
inline void all_draw_p1_init(string &h, unsigned long int head, node &n) {
  all_walker::draw_p1_init(h, head, n);
}
inline void all_draw_p2_init(string &h, unsigned long int head, node &n) {
  all_walker::draw_p2_init(h, head, n);
}
inline void all_draw(string &h, unsigned long int head, node &n) {
  all_walker::draw(h, head, n);
}
inline void all_play(string &h, unsigned long int head, node &n, int c) {
  all_walker::play(h, head, n, c);
}
inline void all_wizard(string &h, unsigned long int head, node &n) {
  all_walker::wizard(h, head, n);
}
inline void all_wizard_self(string &h, unsigned long int head, node &n) {
  all_walker::wizard_self(h, head, n);
}
inline void all_soldior(string &h, unsigned long int head, node &n) {
  all_walker::soldior(h, head, n);
}
