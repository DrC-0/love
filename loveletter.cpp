...
node::node(const std::string &h, bool rnd, const int input_open[3])
  : depth(0), hand1{0, 0}, hand2(0), open1(0), open2(0), deck{n1, n2, n3, n4, n5, n6, n7, n8}, open{0, 0, 0}, hide(0), turn(0), count_turn(0), barrier1(false), barrier2(false) {
  assert(0 < input_open[0] && input_open[0] < 9);
  assert(0 < input_open[1] && input_open[1] < 9);
  assert(0 < input_open[2] && input_open[2] < 9);
  ...
  while(head < h.size()) {
    ...
    case 1:
      key = {action_to_char(c2a1, c2a2)};
      rnd_his.push(key.c_str(), 1);
      rnd_his_p[0].push(key.c_str(), 1);
      org_his.push(key.c_str(), 1);
      org_his_p[0].push(key.c_str(), 1);
      npi[2][depth + 1] = npi[2][depth] * deck[c2a2] * dsum_rec();
      hand1[0] = c2a2 + 1;
      deck[c2a2] -= 1;
      depth++;
    ...
    case 2:
      key = {action_to_char(c2a1, c2a2)};
      rnd_his.push(key.c_str(), 1);
      rnd_his_p[1].push(key.c_str(), 1);
      org_his.push(key.c_str(), 1);
      org_his_p[1].push(key.c_str(), 1);
      npi[2][depth + 1] = npi[2][depth] * deck[c2a2] * dsum_rec();
      hand2 = c2a2 + 1;
      deck[c2a2] -= 1;
      depth++;
    ...
    case 3:
      ...
      key = {action_to_char(c2a1, c2a2)};
      rnd_his.push(key.c_str(), 1);
      rnd_his_p[turn].push(key.c_str(), 1);
      org_his.push(key.c_str(), 1);
      org_his_p[turn].push(key.c_str(), 1);
    ...
    case 4:
      ...
      if(c2a2 + 1 == 1 && barrier2 == false && action.size() > 1) { //兵士宣言
        ...
        org_his.push(key.c_str(), 2);
        org_his_p[0].push(key.c_str(), 2);
        org_his_p[1].push(key.c_str(), 2);
        npi[0][depth + 1] = npi[0][depth];
        npi[1][depth + 1] = npi[1][depth];
        npi[2][depth + 1] = npi[2][depth];
        depth++;
        ...
        rnd_his.push(key.c_str(), 2);
        rnd_his_p[0].push(key.c_str(), 2);
        rnd_his_p[1].push(key.c_str(), 2);
        org_his.push(key.c_str(), 2);
        org_his_p[0].push(key.c_str(), 2);
        org_his_p[1].push(key.c_str(), 2);
        ...
        if(action.size() > 1) {
          ...
          if(to == turn || barrier2 == false) {
            ...
            org_his_p[0].erase();
            org_his_p[1].erase();
            key = {action_to_char(4, 4), wizard_to_char(to, trash, draw)};
            key2 = {action_to_char(4, 4), wizard_to_char(to, trash, 0)};
            rnd_his.push(key.c_str(), 2);
            rnd_his_p[to].push(key.c_str(), 2);
            rnd_his_p[1 - to].push(key2.c_str(), 2);
            org_his.push(key.c_str(), 2);
            org_his_p[to].push(key.c_str(), 2);
            org_his_p[1 - to].push(key2.c_str(), 2);
        ...
        org_his_p[0].erase();
        org_his_p[1].erase();
        key = {action_to_char(4, 5), twonum_to_char(card1 - 1, card2 - 1)};
        key2 = {action_to_char(4, 5), twonum_to_char(card2 - 1, card1 - 1)};
        rnd_his.push(key.c_str(), 2);
        rnd_his_p[turn].push(key.c_str(), 2);
        rnd_his_p[1 - turn].push(key2.c_str(), 2);
        org_his.push(key.c_str(), 2);
        org_his_p[turn].push(key.c_str(), 2);
        org_his_p[1 - turn].push(key2.c_str(), 2);
      }
      break;
    }
  }
...
bool node::operator==(const node &n) const noexcept {
  ...
  string nhistory = n.rnd_his.get_hash_value();
  ...
  history = org_his.get_hash_value();
  nhistory = n.org_his.get_hash_value();
  for(long unsigned int i = 0; i < history.size(); i++) {
    if(history[i] != nhistory[i]) {
      cout << "history[" << i << "] valid " << history[i] << " " << nhistory[i] << endl;
      return false;
  ...
  string history_private[2];
  string nhistory_private[2];
  history_private[0] = rnd_his_p[0].get_hash_value();
  nhistory_private[0] = n.rnd_his_p[0].get_hash_value();
  history_private[1] = rnd_his_p[1].get_hash_value();
  nhistory_private[1] = n.rnd_his_p[1].get_hash_value();
  ...
  history_private[0] = org_his_p[0].get_hash_value();
  nhistory_private[0] = n.org_his_p[0].get_hash_value();
  history_private[1] = org_his_p[1].get_hash_value();
  nhistory_private[1] = n.org_his_p[1].get_hash_value();
  for(long unsigned int i = 0; i < history_private[0].size(); i++) {
    if(history_private[0][i] != nhistory_private[0][i]) {
      cout << "history_private[0] valid " << history_private[0] << " " << nhistory_private[0] << endl;
      return false;
...
void node::do_action(int a, int c, work_do_action &w) {
  ...
  case 5: // play
    ...
    if(card == 1) {
      ...
      if(barrier2 == false) {
        ...
        addstr[0][naddstr[0]++] = twonum_to_char(0, hand2 - 1);
        addstr[1][naddstr[1]++] = action_to_char(4, 1);
        addstr[1][naddstr[1]++] = twonum_to_char(0, hand2 - 1);
        addstr[2][naddstr[2]++] = action_to_char(4, 1);
        addstr[2][naddstr[2]++] = twonum_to_char(0, hand2 - 1);
      } else {
        addstr[0][naddstr[0]++] = action_to_char(4, 1);
        addstr[1][naddstr[1]++] = action_to_char(4, 1);
        addstr[2][naddstr[2]++] = action_to_char(4, 1);
      ...
      if(hand1[0] == hand2 && barrier2 == false) {
        ...
        addstr[0][naddstr[0]++] = twonum_to_char(hand1[0] - 1, hand2 - 1);
        addstr[1][naddstr[1]++] = action_to_char(4, 2);
        addstr[1][naddstr[1]++] = twonum_to_char(hand1[0] - 1, hand2 - 1);
        addstr[2][naddstr[2]++] = action_to_char(4, 2);
        addstr[2][naddstr[2]++] = twonum_to_char(hand1[0] - 1, hand2 - 1);
      } else {
        addstr[0][naddstr[0]++] = action_to_char(4, 2);
        addstr[1][naddstr[1]++] = action_to_char(4, 2);
        addstr[2][naddstr[2]++] = action_to_char(4, 2);
      ...
      if(barrier2 == false) {
        ...
        addstr[0][naddstr[0]++] = twonum_to_char(hand1[0] - 1, hand2 - 1);
        addstr[1 + turn][naddstr[1 + turn]++] = action_to_char(4, 5);
        addstr[1 + turn][naddstr[1 + turn]++] = twonum_to_char(hand1[0] - 1, hand2 - 1);
        addstr[2 - turn][naddstr[2 - turn]++] = action_to_char(4, 5);
        addstr[2 - turn][naddstr[2 - turn]++] = twonum_to_char(hand2 - 1, hand1[0] - 1);
        std::swap(hand1[0], hand2);
        w.prev_open1 = open1;
        w.prev_open2 = open2;
        open1 = hand1[0];
  ...
  case 6: // wizard
    if(barrier2 == false) {
      ...
      org_his_p[1].erase();
      addstr[0][naddstr[0]++] = action_to_char(4, 4);
      addstr[0][naddstr[0]++] = wizard_to_char(1 - turn, hand2 - 1, c - 1);
      addstr[2 - turn][naddstr[2 - turn]++] = action_to_char(4, 4);
      addstr[2 - turn][naddstr[2 - turn]++] = wizard_to_char(1 - turn, hand2 - 1, c - 1);
      addstr[1 + turn][naddstr[1 + turn]++] = action_to_char(4, 4);
      addstr[1 + turn][naddstr[1 + turn]++] = wizard_to_char(1 - turn, hand2 - 1, 0);
      deck[c - 1] -= 1;
      w.prev_hand2 = hand2;
      hand2 = c;
    } else {
      ...
      org_his_p[1].erase();
      addstr[0][naddstr[0]++] = action_to_char(4, 4);
      addstr[0][naddstr[0]++] = wizard_to_char(1 - turn, 7, 7);
      addstr[2 - turn][naddstr[2 - turn]++] = action_to_char(4, 4);
      addstr[2 - turn][naddstr[2 - turn]++] = wizard_to_char(1 - turn, 7, 7);
      addstr[1 + turn][naddstr[1 + turn]++] = action_to_char(4, 4);
      addstr[1 + turn][naddstr[1 + turn]++] = wizard_to_char(1 - turn, 7, 7);
    }
    break;
  case 7: // wizard_self
    // if(0 >= c || c >= 9) print_history();
    ...
    org_his_p[1].erase();
    addstr[0][naddstr[0]++] = action_to_char(4, 4);
    addstr[0][naddstr[0]++] = wizard_to_char(turn, hand1[0] - 1, c - 1);
    addstr[2 - turn][naddstr[2 - turn]++] = action_to_char(4, 4);
    addstr[2 - turn][naddstr[2 - turn]++] = wizard_to_char(turn, hand1[0] - 1, 0);
    addstr[1 + turn][naddstr[1 + turn]++] = action_to_char(4, 4);
    addstr[1 + turn][naddstr[1 + turn]++] = wizard_to_char(turn, hand1[0] - 1, c - 1);
    deck[c - 1] -= 1;
  ...
  case 8: // soldior choise
    ...
    org_his_p[1].erase();
    addstr[0][naddstr[0]++] = action_to_char(4, 0);
    addstr[0][naddstr[0]++] = twonum_to_char(0, c - 1);
    addstr[1][naddstr[1]++] = action_to_char(4, 0);
    addstr[1][naddstr[1]++] = twonum_to_char(0, c - 1);
    addstr[2][naddstr[2]++] = action_to_char(4, 0);
    addstr[2][naddstr[2]++] = twonum_to_char(0, c - 1);
    break;
  }
  depth++;
...
void node::undo_action(int a, int c, work_do_action &w) {
  ...
  case 4: // draw
    ...
    count_turn--;
    rnd_his.erase();
    rnd_his_p[turn].erase();
    org_his.erase();
    org_his_p[turn].erase();
